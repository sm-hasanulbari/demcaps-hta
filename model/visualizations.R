  ...
  theme_minimal(base_size = base_size) +
    theme(
      plot.title       = element_text(
        ...
        colour = "#1C2833", margin = margin(b = 8)),
      plot.subtitle    = element_text(
        size = base_size - 1, colour = "#566573",
        margin = margin(b = 12)),
      plot.caption     = element_text(
        size = base_size - 3, colour = "#99A3A4",
        hjust = 0, margin = margin(t = 10)),
      axis.title       = element_text(
        face = "bold", colour = "#2C3E50",
        size = base_size - 1),
      axis.text        = element_text(colour = "#566573"),
      panel.grid.major = element_line(
        colour = "#EAECEE", linewidth = 0.5),
...
plot_trace <- function(trace,
  ...
  p <- ggplot(df_long,
    ...
    labs(
      ...
      subtitle = "Proportion of cohort per annual cycle",
      x        = "Model Cycle (Years)",
      y        = "Proportion of Cohort",
      fill     = "Health State",
      caption  = "Handels et al. (2017)[5] | GERAS-EU[6]"
    ) +
    theme_demcaps()
...
plot_ce_plane <- function(psa_df,
  ...
  df <- psa_df %>%
    ...
    dplyr::mutate(Arm = factor(Arm,
                               levels = c("Psychosocial", "Technology",
                                          "Combination")))
  ...
  p <- ggplot(df,
    ...
    geom_point(alpha = 0.35, size = 1.5,
               shape = 16) +
    stat_summary(aes(group = Arm), fun = mean,
                 geom = "point", size = 5,
                 shape = 18, show.legend = FALSE) +
    scale_colour_manual(
      values = demcaps_palette[c(
    ...
    labs(
      ...
      subtitle = paste0("PSA scatter (n=",
                        ...
                        "); dashed = WTP threshold"),
      x        = "Incremental QALYs",
      y        = "Incremental Costs (EUR)",
      colour   = "Intervention",
      caption  = "WTP: Dutch guidelines (ZIN 2024)[3]"
    ) +
    theme_demcaps() +
    annotate("text", x = Inf, y = Inf,
             label    = paste0("WTP = EUR ",
                               formatC(wtp_line, format = "d",
                                       big.mark = ",")),
             hjust    = 1.1, vjust = 2,
             colour   = "#E67E22",
             size     = 3.5,
...
plot_ceac <- function(ceac_df, use_plotly = TRUE) {
  ...
  p <- ggplot(ceac_df,
    ...
    geom_vline(xintercept = 80000,
               ...
               linewidth = 0.8,
               alpha     = 0.9) +
    annotate("text", x = 20000, y = 0.88,
             label    = "EUR 20k",
             hjust    = -0.1, size = 3.2,
             colour   = "#E67E22",
             fontface = "italic") +
    annotate("text", x = 50000, y = 0.88,
             label    = "EUR 50k",
             hjust    = -0.1, size = 3.2,
             colour   = "#E74C3C",
             fontface = "italic") +
    annotate("text", x = 80000, y = 0.88,
             label    = "EUR 80k",
             hjust    = -0.1, size = 3.2,
             colour   = "#8E44AD",
             fontface = "italic") +
    ...
    labs(
      ...
      colour   = "Strategy",
      caption  = paste0(
        "Orange=EUR 20k | Red=EUR 50k | ",
        "Purple=EUR 80k (Dutch thresholds)[3]")
    ) +
    theme_demcaps()
...
plot_tornado <- function(dsa_df,
                         use_plotly = TRUE) {
  
  df <- dsa_df %>%
    dplyr::arrange(ICER_Range) %>%
    dplyr::mutate(
  ...
  p <- ggplot(df) +
    ...
    labs(
      ...
      subtitle = "One-way sensitivity — Psychosocial ICER",
      x        = "ICER (EUR per QALY)",
      y        = NULL,
      caption  = paste0(
        "Red dashed = base-case ICER | ",
        "Orange = EUR 20k | ",
        "Purple = EUR 50k (Dutch thresholds)[3]")
    ) +
    theme_demcaps() +
...
plot_nmb <- function(results_df,
  ...
  p <- ggplot(df,
    ...
    labs(
      title    = paste0(
        "Net Monetary Benefit at EUR ",
        formatC(wtp, format = "d",
                big.mark = ","), "/QALY"),
      subtitle = "Positive NMB = cost-effective vs SoC",
      x        = NULL,
      y        = "NMB (EUR)",
  ...
  if (use_plotly) ggplotly(p, tooltip = "text")
  else p
}
  ...
  p <- ggplot(df,
    ...
    labs(
      ...
      subtitle = paste0(
        "20-year horizon | ",
        discount_costs * 100,
        "% discount | Societal perspective"),
      x        = NULL,
      y        = "Total Discounted Cost (EUR)",
      caption  = "ZIN 2024[3] | Alzheimer Nederland[14]"
    ) +
    theme_demcaps() +
    theme(legend.position = "none")
